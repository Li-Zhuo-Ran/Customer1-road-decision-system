...
# --- M3 养护决策模块 (基于专家知识库) ---
    ...
    if st.session_state.processed_data is None:
        ...
        st.markdown('### 规则层（Apriori 关联规则 + PLSR 回归）')
        with st.expander('🔗 规则层（在决策前进行关联/回归辅助分析）', expanded=False):
            st.write('说明：Apriori 用于挖掘定性条件间的关联规则；PLSR 用于定量变量间的线性回归分析。')
            dfs_for_rule = df.copy()
            # --- Apriori 部分 ---
            st.markdown("#### Apriori 关联规则挖掘（定性/二值化后）")
            apriori_cols = st.multiselect("选择用于Apriori挖掘的字段（会被二值化/编码）", options=dfs_for_rule.columns.tolist(), key='r_apriori_cols')
            if apriori_cols:
                trans = pd.DataFrame(index=dfs_for_rule.index)
                for c in apriori_cols:
                    if pd.api.types.is_numeric_dtype(dfs_for_rule[c]):
                        med = float(dfs_for_rule[c].median()) if dfs_for_rule[c].notna().any() else 0.0
                        thr = st.number_input(f"数值列 {c} 的二值化阈值 (判定为 True if > thresh)", value=med, key=f"r_thr_{c}")
                        trans[f"{c}_gt_{thr}"] = dfs_for_rule[c] > float(thr)
                    else:
                        vals = dfs_for_rule[c].astype(str)
                        topk = vals.value_counts().index.tolist()[:10]
                        sel = st.multiselect(f"选择 {c} 的类别用于编码（多选）", options=topk, key=f"r_sel_{c}")
                        for sv in sel:
                            trans[f"{c}={sv}"] = vals == sv

                if not trans.empty:
                    min_sup = st.slider("最小支持度 (support)", 0.01, 1.0, 0.1, key='r_min_sup')
                    min_conf = st.slider("最小置信度 (confidence)", 0.01, 1.0, 0.6, key='r_min_conf')
                    try:
            ...
            numeric_cols_r = [c for c in dfs_for_rule.columns if pd.api.types.is_numeric_dtype(dfs_for_rule[c])]
            st.write("可用数值字段：", numeric_cols_r)
            pls_x = st.multiselect("选择自变量 X（数值列）", options=numeric_cols_r, default=numeric_cols_r[:min(5,len(numeric_cols_r))], key='r_pls_x')
            pls_y = st.selectbox("选择因变量 y（数值列）", options=numeric_cols_r, index=0, key='r_pls_y')
            max_comp = min(10, max(1, len(pls_x)))
            max_comp = max_comp if max_comp>0 else 1
            n_comp = st.slider("PLSR 主成分数 (n_components)", 1, max_comp, min(2, max_comp), key='r_pls_comp')
            if st.button("运行 PLSR 分析", key='r_run_plsr'):
                if not pls_x or pls_y is None:
                    st.error("请先选择 X 与 y 字段")
                else:
                    X = dfs_for_rule[pls_x].fillna(0).values
                    y = dfs_for_rule[pls_y].fillna(0).values
                    try:
        ...
        if not df_year.empty:
            ...
            if st.button('生成详细技术方案'):
                ...
                with st.expander('🔍 NSGA 多目标优化（养护效益↑ / 成本↓）', expanded=False):
                    ...
                    df_opt_source = df_year.copy()
                    max_candidates = min(50, len(df_opt_source))
                    top_k = st.slider('选择待优化路段数量 (按健康分低→高)', 1, max_candidates, min(10, max_candidates), key='opt_top_k')
                    df_sorted = df_opt_source.sort_values('预测健康分')
                    candidates = df_sorted['路段ID'].unique().tolist()[:top_k]
                    st.write(f'将对以下 {len(candidates)} 个路段进行组合优化（可在表中修改选择）')
                    chosen = st.multiselect('选择要优化的路段', candidates, default=candidates, key='opt_choose')
                    ...
                    sel_df = df_opt_source[df_opt_source['路段ID'].isin(chosen)].copy()
                    m = len(sel_df)
                    if m == 0:
                        st.info('未选择任何路段进行优化')
                    else:
                        ...
                        st.write('示例：首行路段的动作收益/成本')
                        st.write(pd.DataFrame({'动作':actions, '收益示例':benefit_mat[0].round(2), '成本示例':cost_mat[0].round(2)}))

                        # 可调整的 NSGA 参数
                        pop_size = st.number_input('NSGA 种群大小 (pop_size)', min_value=4, max_value=200, value=40, step=4, key='opt_pop_size')
                        n_gen = st.number_input('NSGA 迭代代数 (n_gen)', min_value=1, max_value=200, value=40, step=1, key='opt_n_gen')
                        run_nsga = st.button('运行 NSGA-II 优化', key='run_nsga_m4')
                        if run_nsga:
                            st.info('已触发 NSGA-II，准备运行（可能需要几秒至数分钟）')
                            try:
                                with st.spinner('NSGA-II 运行中...'):
                                    ...
                                    res = minimize(prob, algorithm, ('n_gen', int(n_gen)), seed=1, verbose=False)
                                    ...
                                    if not res_df.empty:
                                        pick = st.selectbox('选择要导出的方案（按行）', res_df['解号'].tolist(), key='pick_nsga')
                                        sel_row = res_df[res_df['解号']==pick].iloc[0]
                                        if st.button('将选定方案应用到 processed_data 并生成导出', key='apply_nsga'):
                                            ...
                                            st.session_state.processed_data = df_all
                                            st.success('已将选定方案写入 processed_data，可在本页或 M5 查看效果')
                            except Exception as e:
                                st.error(f'NSGA 运行失败: {e}')
        else:
...
# --- M5 效果评估模块 (P1) ---
    ...
    st.subheader('基于已生成的决策，展示效果汇总与可视化')
    if st.session_state.processed_data is None:
        st.warning('请先在 M3 生成决策方案并保存到 processed_data')
    else:
        df_eval = st.session_state.processed_data.copy()
        # 快速指标卡
        avg_health = df_eval['预测健康分'].mean() if '预测健康分' in df_eval.columns else None
        median_health = df_eval['预测健康分'].median() if '预测健康分' in df_eval.columns else None
        total_segments = df_eval['路段ID'].nunique() if '路段ID' in df_eval.columns else len(df_eval)
        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric('路段数量', int(total_segments))
        with c2:
            st.metric('平均预测健康分', f"{avg_health:.2f}" if avg_health is not None else 'N/A')
        with c3:
            st.metric('健康分中位数', f"{median_health:.2f}" if median_health is not None else 'N/A')

        st.markdown('---')
        # 决策分布与示例效果
        if '决策方案' in df_eval.columns:
            st.subheader('决策方案分布')
            dist = df_eval['决策方案'].value_counts().reset_index()
            dist.columns = ['方案','数量']
            fig = px.pie(dist, names='方案', values='数量', title='各类养护方案分布')
            st.plotly_chart(fig, use_container_width=True)

            st.subheader('各方案平均健康分')
            mean_by_scheme = df_eval.groupby('决策方案')['预测健康分'].mean().reset_index()
            fig2 = px.bar(mean_by_scheme, x='决策方案', y='预测健康分', color='决策方案', title='按方案分组的平均预测健康分')
            st.plotly_chart(fig2, use_container_width=True)

        else:
            st.info('processed_data 中尚未包含 `决策方案` 字段，先在 M3 中生成决策')

        # 简单对比：若存在原始 PCI/PSSI 与预测值，展示对比图
        if set(['PCI','预测PCI']).issubset(df_eval.columns):
            st.subheader('PCI 原始 vs 预测 对比（示例）')
            sample = df_eval[['路段ID','PCI','预测PCI']].dropna().head(50)
            sample_m = sample.melt(id_vars=['路段ID'], value_vars=['PCI','预测PCI'], var_name='指标', value_name='值')
            fig3 = px.bar(sample_m, x='路段ID', y='值', color='指标', barmode='group')
            st.plotly_chart(fig3, use_container_width=True)

# R1 功能已并入 M3 界面（规则层作为决策模块内的辅助展开项）
